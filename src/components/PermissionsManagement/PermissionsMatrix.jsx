  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-500">View permissions assigned to each role</p>
      </div>
            ...
            {Object.entries(groupedPermissions).map(([category, categoryPermissions]) => (
              <React.Fragment key={category}>
                <tr className="bg-gray-100">
                  <td colSpan={roles.length + 1} className="px-6 py-2 text-sm font-medium text-gray-900 sticky left-0 bg-gray-100 z-10">
                    {category}
                  </td>
                </tr>
                {categoryPermissions.map((permission, index) => (
                  <tr key={permission.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-6 py-3 text-sm text-gray-900 sticky left-0 bg-inherit z-10 border-r border-gray-200">
                      <div>
