...
const ProjectDetails = () => {
    ...
    return (
      <div className="card p-6 sm:p-12 text-center">
        ...
        <button
          ...
          className="btn-primary mt-4 px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm"
        >
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center space-x-3 sm:space-x-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-gray-500 hidden sm:block">{project.description}</p>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          <span className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${getStatusBadge(project.status)}`}>
            {project.status}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <div className="card p-4 sm:p-6">
          <div className="flex items-center">
            ...
            <div className="ml-3 sm:ml-4">
              ...
              <p className="text-xl sm:text-2xl font-semibold text-gray-900">{calculateProjectProgress()}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="card p-4 sm:p-6">
          <div className="flex items-center">
            ...
            <div className="ml-3 sm:ml-4">
              ...
              <p className="text-xl sm:text-2xl font-semibold text-gray-900">{project.spans.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="card p-4 sm:p-6">
          <div className="flex items-center">
            ...
            <div className="ml-3 sm:ml-4">
              <p className="text-xs sm:text-sm font-medium text-gray-500">Budget Spent</p>
              <p className="text-xl sm:text-2xl font-semibold text-gray-900">
      ...
      <div className="card p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-medium text-gray-900">Area Manager</p>
              <p className="text-xs sm:text-sm text-gray-500 truncate">{project.assignedAreaManager}</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-medium text-gray-900">Start Date</p>
              <p className="text-xs sm:text-sm text-gray-500">{formatDate(project.startDate)}</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            <Calendar className="h-4 w-4 sm:h-5 sm:w-5 text-gray-400 flex-shrink-0" />
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-medium text-gray-900">End Date</p>
              <p className="text-xs sm:text-sm text-gray-500">{formatDate(project.endDate)}</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-medium text-gray-900">Total Budget</p>
              <p className="text-xs sm:text-sm text-gray-500">{formatCurrency(project.budget)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Spans and Nodes */}
      <div className="space-y-4 sm:space-y-6">
        <h2 className="text-base sm:text-lg font-semibold text-gray-900">Project Structure</h2>
        
        {project.spans.map((span) => (
          <div key={span.id} className="card p-4 sm:p-6">
            {/* Span Header */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div className="min-w-0">
                  <h3 className="text-base sm:text-lg font-semibold text-gray-900 truncate">{span.name}</h3>
                  <p className="text-xs sm:text-sm text-gray-500">{span.description}</p>
                </div>
              </div>
              <div className="flex items-center space-x-2 sm:space-x-4">
                ...
                >
                  {selectedSpan === span.id ? 'Hide' : 'Show'}
                </button>
              </div>
            </div>
            ...
            {selectedSpan === span.id && (
              <div className="border-t border-gray-200 pt-4 space-y-4">
                ...
                <div className="space-y-3 sm:space-y-4">
                  <h4 className="text-sm sm:text-base font-medium text-gray-900">Nodes</h4>
                  {span.nodes.map((node) => (
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                        <div className="flex items-center space-x-2 sm:space-x-3">
                          <div className="w-6 h-6 sm:w-8 sm:h-8 bg-purple-100 rounded-lg flex items-center justify-center flex-shrink-0">
                            <Settings className="h-3 w-3 sm:h-5 sm:w-5 text-purple-600" />
                          </div>
                          <div className="min-w-0">
                            <h5 className="text-xs sm:text-sm font-semibold text-gray-900 truncate">{node.name}</h5>
                            <p className="text-xs text-gray-500 truncate">{node.gp} • {node.description}</p>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2 sm:space-x-4">
                          <div className="flex items-center space-x-2">
